...
type RestOf<T extends IShift | NewShift> = Omit<T, 'shiftDate'> & {
...
export class ShiftService {
  ...
  protected convertDateFromClient<T extends IShift | NewShift | PartialUpdateShift>(shift: T): RestOf<T> {
    return {
      ...shift,
      shiftDate: shift.shiftDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restShift: RestShift): IShift {
    return {
      ...restShift,
      shiftDate: restShift.shiftDate ? dayjs(restShift.shiftDate) : undefined,
    };
  }
