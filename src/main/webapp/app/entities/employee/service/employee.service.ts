...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'dateOfBirth' | 'hireDate' | 'terminationDate'> & {
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...employee,
      dateOfBirth: employee.dateOfBirth?.toJSON() ?? null,
      hireDate: employee.hireDate?.toJSON() ?? null,
      terminationDate: employee.terminationDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...
      terminationDate: restEmployee.terminationDate ? dayjs(restEmployee.terminationDate) : undefined,
    };
  }
