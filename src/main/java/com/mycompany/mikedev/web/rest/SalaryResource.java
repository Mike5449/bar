...
/**
 * REST controller for managing {@link com.mycompany.mikedev.domain.Salary}.
 */
@RestController
@RequestMapping("/api")
public class SalaryResource {
    ...
    public ResponseEntity<SalaryDTO> updateSalary(
        ...
        SalaryDTO result = salaryService.update(salaryDTO);
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, salaryDTO.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<SalaryDTO> partialUpdateSalary(
        @PathVariable(value = "id", required = false) final Long id,
        ...
        Optional<SalaryDTO> result = salaryService.partialUpdate(salaryDTO);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, salaryDTO.getId().toString())
        );
    }
    ...
    public ResponseEntity<List<SalaryDTO>> getAllSalaries(
        ...
        Page<SalaryDTO> page;
        if (eagerload) {
            page = salaryService.findAllWithEagerRelationships(pageable);
        } else {
            page = salaryService.findAll(pageable);
        }
